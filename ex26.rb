# Exercise 26: Congratulations, Take A Test (Refactoring)!
...
def sort_words(words)
    words.sort()
    return words.sort() # changed from: sorted(words)
end

# Prints the first word after popping it off.
def puts_first_word(words)
    word = words.pop(0) # changed from .poop(0)
    puts word
end

# Prints the last word after popping it off.
def puts_last_word(words)
    word = words.pop() # changed from: (-1)
    puts word
end
